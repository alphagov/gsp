...
AWS_REDIS_SECURITY_GROUP_ID=$(aws ec2 describe-security-groups | jq -r '.SecurityGroups[] | select(.GroupName == "sandbox_redis_from_worker") | .GroupId')
...
docker build \
	...
	--build-arg AWS_REDIS_SECURITY_GROUP_ID=$AWS_REDIS_SECURITY_GROUP_ID \
	--build-arg AWS_REDIS_SUBNET_GROUP_NAME=sandbox-private \
