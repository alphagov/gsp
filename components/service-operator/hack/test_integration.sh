...
AWS_ELASTICACHE_CLUSTER_SECURITY_GROUP_ID=$(aws ec2 describe-security-groups | jq -r '.SecurityGroups[] | select(.GroupName == "sandbox_elasticache_cluster_from_worker") | .GroupId')
...
docker build \
	...
	--build-arg AWS_ELASTICACHE_CLUSTER_SECURITY_GROUP_ID=$AWS_ELASTICACHE_CLUSTER_SECURITY_GROUP_ID \
	--build-arg AWS_ELASTICACHE_CLUSTER_SUBNET_GROUP_NAME=sandbox-private
