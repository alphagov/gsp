...
var (
	log        = logf.Log.WithName("concourse-operator")
...
func Add(mgr manager.Manager) error {
	...
	if len(secretName) == 0 {
		secretName = "concourse-operator-webhook-secret"
	}

	svr, err := webhook.NewServer("pipeline-validation-server", mgr, webhook.ServerOptions{
		Port:    9876,
		CertDir: "/tmp/cert",
		BootstrapOptions: &webhook.BootstrapOptions{
			...
			Service: &webhook.Service{
				Namespace: ns,
				Name:      "concourse-webhook-service",
