...
resource "aws_cloudformation_stack" "ci-nodes" {
  ...
  parameters = {
    ...
    NodeVolumeSize                      = "75"
    BootstrapArguments                  = "--kubelet-extra-args \"--node-labels 'node-role.kubernetes.io/ci=true,generation=${var.worker_generation_timestamp}' --register-with-taints=node-role.kubernetes.io/ci=:NoSchedule --event-qps=0\""
    VpcId                               = var.vpc_id
    Subnets                             = join(",", var.private_subnet_ids)
  }
