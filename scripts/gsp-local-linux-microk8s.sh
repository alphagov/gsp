...
function apply() {
	...
	until microk8s.kubectl --kubeconfig="$HOME/.kube/config" apply -R -f "${1}"; do
	...
	until ! microk8s.kubectl --kubeconfig="$HOME/.kube/config" get pods --all-namespaces | tail -n "+2" | grep -v Completed | grep -v Running; do
...
microk8s.kubectl --kubeconfig="$HOME/.kube/config" config set-context --current --namespace gsp-system
...
microk8s.kubectl --kubeconfig="$HOME/.kube/config" cluster-info
log "Local GSP ready."
